...
def show_dashboard():
    ...
    with st.sidebar:
        ...
        for cat in 분류1_list:
            ...
            if st.button(cat, key=f"sidebar_btn_{cat}", use_container_width=True, type="primary" if is_active else "secondary"):
...
def show_category_detail(df, 분류1):
    ...
    for row in range(num_rows):
        ...
        end_idx = min(start_idx + buttons_per_row, len(분류2_list))
        
        for idx, col in enumerate(cols):
            if start_idx + idx < end_idx:
                cat2 = 분류2_list[start_idx + idx]
                with col:
                    is_active = (st.session_state[f'selected_분류2_{분류1}'] == cat2)
                    if st.button(cat2, key=f"btn2_{분류1}_{cat2}_{row}_{idx}", use_container_width=True, type="primary" if is_active else "secondary"):
    ...
    with col3:
        if st.button("📋 테이블", key=f"table_mode_{분류1}", use_container_width=True):
            ...
            st.rerun()
    with col4:
        if st.button("🎯 드래그", key=f"drag_mode_{분류1}", use_container_width=True):
            st.session_state[f'view_mode_{분류1}'] = 'drag'
            st.rerun()
    ...
    # 현재 모드 확인
    if f'view_mode_{분류1}' not in st.session_state:
        st.session_state[f'view_mode_{분류1}'] = 'table'
    
    current_mode = st.session_state[f'view_mode_{분류1}']
