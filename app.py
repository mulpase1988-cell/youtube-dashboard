...
def show_category_detail(df, 분류1):
    ...
    # [수정] '운영 기간' 컬럼 추가 (분류2와 동영상 사이)
    display_columns = ['채널명', '분류2', '운영 기간', '동영상', '조회수', '최근 5개 토탈', '최근 10개 토탈', '최근 20개 토탈', '최근 30개 토탈', 'URL']
    df_fmt = df_display[[c for c in display_columns if c in df_display.columns]].copy()
    
    # 숫자 데이터 포맷팅 ('운영 기간'은 텍스트일 것이므로 제외됨)
    for col in df_fmt.columns:
        if col not in ['URL', '운영 기간'] and df_fmt[col].dtype in ['int64', 'float64']: 
...
def show_settings():
    ...
    with tab1:
        ...
        if new_order != current_list: st.session_state.page_order['분류1_순서'] = new_order; st.rerun()
    with tab2:
        ...
        if new_sub != current_sub: st.session_state.page_order['분류2_순서'][selected_cat1] = new_sub; st.rerun()
    with tab3:
        if st.button("💾 구글 시트에 최종 저장", type="primary", use_container_width=True):
            if save_config_to_sheet(st.session_state.page_order): st.success("✅ 저장 완료!"); st.cache_data.clear()
        if st.button("🗑️ 모든 채널 개별 순서 초기화", use_container_width=True): st.session_state.page_order['채널_순서'] = {}; st.rerun()
